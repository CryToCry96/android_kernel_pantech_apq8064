...
static ssize_t msm_fb_check_panel_power_state(struct device *dev,
		                          struct device_attribute *attr, char *buf)
{
	ssize_t ret = 0;
	struct fb_info *fbi = dev_get_drvdata(dev);
...
static ssize_t msm_fb_set_panel_power_state(struct device *dev,
...
#else
	if (panel_onoff != mfd->panel_power_state) {
		if (panel_onoff == 1) {
...
static DEVICE_ATTR(panel_power_state, S_IRUGO | S_IWUSR, msm_fb_check_panel_power_state, msm_fb_set_panel_power_state);
#endif
